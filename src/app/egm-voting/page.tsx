          ...
          {data ? <>
            ...
            {data.meeting.status === "register_open" ? <section className="grid gap-4 lg:grid-cols-[1fr_1.5fr]">
              <form onSubmit={addAttendee} className="rounded-2xl bg-white p-5 shadow-sm">
                ...
                <div className="mt-2 flex gap-2"><input type="datetime-local" value={meetingAt} onChange={(event) => setMeetingAt(event.target.value)} className="min-w-0 flex-1 rounded-xl border border-slate-300 px-3 py-2" /><button type="button" disabled={busy} onClick={() => void mutate({ action: "set_meeting", meetingAt: meetingAt ? new Date(meetingAt).toISOString() : "" }, "Meeting date saved.")} className="rounded-xl border border-teal-600 px-4 py-2 font-bold text-teal-800">Save</button></div>
