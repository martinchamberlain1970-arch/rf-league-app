...
export default function CaptainResultsPage() {
  ...
  const loadAll = useCallback(async () => {
    ...
    const memberPromise = (async () => {
      const result = await fetchAllSupabasePages<TeamMember>((from, to) =>
        client
          ...
          .select("season_id,team_id,player_id,is_captain,is_vice_captain,is_match_scorer")
      ...
      const fallback = await fetchAllSupabasePages<Omit<TeamMember, "is_match_scorer">>((from, to) =>
        client
          .from("league_team_members")
          .select("season_id,team_id,player_id,is_captain,is_vice_captain")
          .order("season_id", { ascending: true })
          .order("team_id", { ascending: true })
          .order("player_id", { ascending: true })
          .range(from, to)
    ...
    const [seasonRes, teamRes, memberRes, initialFixtureRes, pendingRes, playerRes] = await Promise.all([
      fetchAllSupabasePages<Season>((from, to) =>
        client
          .from("league_seasons")
      ...
      fetchAllSupabasePages<Team>((from, to) =>
        client.from("league_teams").select("id,season_id,name").order("id", { ascending: true }).range(from, to)
      ),
      memberPromise,
      ...
      fetchAllSupabasePages<PendingSubmission>((from, to) =>
        client
          .from("league_result_submissions")
          .select("id,fixture_id,status,frame_results,scorecard_photo_url")
          .eq("status", "pending")
      ...
      fetchAllSupabasePages<Player>((from, to) =>
        client
          .from("players")
          .select("id,display_name,full_name,snooker_handicap,rating_snooker")
    ...
    if (initialFixtureRes.error && (initialFixtureRes.error.message.toLowerCase().includes("pre_match_") || initialFixtureRes.error.message.toLowerCase().includes("proxy_entry"))) {
      const fallbackFixtureRes = await fetchAllSupabasePages<Fixture>((from, to) =>
        client
          .from("league_fixtures")
    ...
    const slotRes = relevantFixtureIds.length
      ? await fetchAllSupabasePagesByChunks<FrameSlot, string>(relevantFixtureIds, (fixtureIdChunk, from, to) =>
          client
  ...
  const lineupNextAction = preMatchPaperRecord
    ...
    : awayLineupSubmitted
      ...
      : homeLineupSubmitted
        ...
        : `Home captain should enter frames 1-${slots.length || 5} first and send them to the opponent.`;
  ...
  const applySinglesSelection = async (slot: FrameSlot, side: "home" | "away", selection: string) => {
    ...
    if (selection === "__NO_SHOW__") {
      if (isWinterFormat && slot.slot_no === 3) {
        ...
        let frameFour = activeSlots.find((row) => row.slot_type === "singles" && row.slot_no === 4);
        let doublesFrame = activeSlots.find((row) => row.slot_type === "doubles");
        if (!frameFour || !doublesFrame) {
          const sessionRes = await supabase?.auth.getSession();
          const token = sessionRes?.data.session?.access_token;
          if (!token || !selectedFixture) {
            setMessage("The complete winter scorecard could not be loaded. Refresh the page and try again.");
            return;
          }
          const repairRes = await fetch("/api/league/ensure-fixture-frames", {
            method: "POST",
            headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
            body: JSON.stringify({ fixtureId: selectedFixture.id }),
          });
          ...
          if (!repairRes.ok) {
            setMessage(repairBody?.error ?? "The complete winter scorecard could not be loaded. Refresh the page and try again.");
            return;
          }
          const repairedFrames = (repairBody?.frames ?? []) as FrameSlot[];
          const localById = new Map(slots.map((row) => [row.id, row]));
                    ...
                    <section className="rounded-2xl border border-sky-200 bg-white p-4 shadow-sm">
                      ...
                      <div className="mt-3 grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
                        <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-3">
                          ...
                          <p className="mt-2 text-sm text-slate-700">{proxyEntryEnabled ? "Enter the home lineup first so the fixture order stays consistent before you confirm both teams." : `Frames 1-${slots.length} should be submitted to the opponent by 19:15.`}</p>
                        </div>
                      ...
                      <div className="mt-4 rounded-xl border border-sky-200 bg-sky-50/70 p-3">
                        <p className="text-sm font-semibold text-slate-900">Lineup actions</p>
                        <p className="mt-1 text-xs text-slate-600">Review all five frames above, then choose the appropriate next action.</p>
                        <div className="mt-3 flex flex-wrap gap-2">
                          {selectedFixtureSide === "home" && lineupWindowOpen && !awayLineupSubmitted ? (
                          ...
                          {canEditSubmittedHomeLineup ? (
                            <button type="button" onClick={async () => {
                              if (await showConfirm({ title: "Reopen the home lineup?", description: "The submitted home lineup will reopen for editing. This is only available before 19:15 and before the away team confirms its lineup.", confirmLabel: "Reopen lineup" })) void reopenSubmittedHomeLineup();
                            }} disabled={submitting} className="rounded-xl border border-amber-300 bg-amber-50 px-4 py-2 text-sm font-medium text-amber-900">Edit submitted home lineup</button>
                          ) : null}
                          {canSubmitAwayLineup ? (
                            <button type="button" onClick={() => void submitLineupForSide("away")} disabled={submitting} className="rounded-xl bg-sky-700 px-4 py-2 text-sm font-semibold text-white">
