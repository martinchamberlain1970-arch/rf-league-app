  ...
  return (
    <main className="min-h-screen bg-slate-100 p-4 sm:p-6">
      <div className="mx-auto max-w-6xl space-y-4">
        <RequireAuth>
          <ScreenHeader title="Team Entry Packs" eyebrow="League Administration" subtitle="Issue private public links, review team rosters, and import league players." />
          ...
          {admin.canManageLeague ? <>
            ...
            <section className="rounded-2xl border border-sky-200 bg-sky-50 p-5 shadow-sm">
              ...
              <ol className="mt-3 list-decimal space-y-2 pl-5 text-sm text-slate-800">
                ...
                <li>The team saves a draft while collecting every player’s required match telephone number.</li>
                ...
                <li>Competition entries are collected separately using the public competition-entry form.</li>
            ...
            <section className="space-y-3">
              ...
              {packs.map((pack) => {
                ...
                return <article key={pack.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  ...
                  <div className="mt-3 grid gap-3 sm:grid-cols-2"><div className="rounded-xl border border-slate-200 p-3"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Players</p><p className="mt-1 text-2xl font-black text-slate-950">{players.length}</p></div><div className="rounded-xl border border-slate-200 p-3"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Last updated</p><p className="mt-1 text-sm font-bold text-slate-950">{new Date(pack.updated_at).toLocaleString()}</p></div></div>
