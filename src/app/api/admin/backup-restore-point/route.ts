...
export async function POST(req: NextRequest) {
  ...
  if (!path || !path.endsWith(".json")) {
    return NextResponse.json({ error: "Invalid restore-point path." }, { status: 400 });
  }

  const adminClient = createClient(supabaseUrl as string, serviceRoleKey as string);
  const fileRes = await adminClient.storage.from("backups").download(path);
  if (fileRes.error || !fileRes.data) {
    return NextResponse.json({ error: fileRes.error?.message ?? "Failed to download restore point." }, { status: 400 });
  }

  let payload: Record<string, unknown>;
  try {
    payload = JSON.parse(await fileRes.data.text()) as Record<string, unknown>;
  } catch {
    return NextResponse.json({ error: "Restore-point file is not valid JSON." }, { status: 400 });
  }
  ...
  return NextResponse.json({ ok: true, restored, skipped, path });
}
