...
export async function POST(req: NextRequest) {
  ...
  try { await requireSuperUser(adminClient, auth.data.user); } catch { return NextResponse.json({ error: "Only the Super User can assign protected roles." }, { status: 403 }); }
  ...
  if (userId === auth.data.user.id) return NextResponse.json({ error: "The Super User cannot change their own protected role here." }, { status: 400 });
  ...
  if (previousRole === "owner" || previousRole === "super") return NextResponse.json({ error: "Another protected Super User role cannot be changed here." }, { status: 403 });
