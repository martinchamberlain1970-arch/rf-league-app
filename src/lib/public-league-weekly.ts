...
export async function buildPublicWeeklyHandicapReview(
  ...
  const reviewNote =
    !isInformationOnly && selectedWeek === 2
      ? "System correction: the app and the automatic database review were rounding exact halfway Elo values differently. The live playing handicaps and Elo results were correct, but the Target from Elo column could show a false four-point gap and the weekly review could show a handicap movement that had not actually occurred. The calculation is now consistent throughout the system: for example, Elo 1029 remains in the -4 band and Elo 1030 begins the -8 band. The affected display and Week 2 audit presentation have been corrected; no match results or Elo values were changed. The scheduled reviews during the first four weeks will ensure that any remaining minor discrepancies affecting a small number of players are resolved by the end of Week 4. From Week 3 onwards, Premier League doubles results will carry 50% of the normal Elo weighting. This reflects that doubles team handicaps are calculated from the two players’ combined handicaps divided by two, and recognises that a doubles result is shared with a partner. Weeks 1 and 2 will not be recalculated retrospectively. If you have any questions, please contact Martin Chamberlain, League Secretary."
